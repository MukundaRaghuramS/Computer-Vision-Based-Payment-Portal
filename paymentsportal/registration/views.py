...
import qrcode
import cv2
from .eye_control import eye_control
import threading
...
def settings_view(request):
    if request.method == 'POST':
        ...
        request.session['theme'] = selected_theme  # Save the selected theme
        return redirect('settings')  # Redirect to the settings page to apply changes
...
def toggle_eye_control(request):
    if request.method == 'POST':
        if eye_control.running:
            print("Stopping eye control.")
            eye_control.stop()  # Stop eye control if it's running
        else:
            print("Starting eye control.")
            eye_control.start()  # Start eye control if it's not running
    return redirect('home')